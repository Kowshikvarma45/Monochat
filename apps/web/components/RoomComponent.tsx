...
const RoomComponent: React.FC<RoomProps> = ({ roomname, creator, createdAt, onJoin }) => {
  ...
  return (
    <div className="group relative bg-[#232428]/80 backdrop-blur-lg border border-gray-950 rounded-xl shadow-md p-5 w-full max-w-lg transition-all hover:shadow-xl">
      ...
      <button 
        className={`mt-4 w-full bg-green-600 hover:bg-green-800 text-white font-bold py-2 rounded-lg shadow-md transition-all duration-200 ${joinloading?"animate-none":"animate-none"}`}
        onClick={() =>{
          setjoinloading(true)
          router.push(onJoin)
        } }
